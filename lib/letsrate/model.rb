...
module Letsrate
  ...
  def rate(stars, user_id, dimension=nil)
    if can_rate? user_id, dimension
      ...
      existing_rates = rates(dimension).where(rater_id: user_id)
  ...
  def update_rate_average(stars, dimension=nil)
    if average(dimension).nil?
      ...
      #we get the current average based on all the current rating values
      current_rates = rates(dimension)
      avg = average(dimension)
      avg.avg = current_rates.map(&:stars).sum/current_rates.count
      avg.qty = current_rates.count
      avg.save!
    end
  end
